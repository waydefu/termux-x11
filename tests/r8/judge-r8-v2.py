#!/usr/bin/env python3
"""Judge one Gate A R8 lifecycle cell (observation-stream amendment).

Explicit successor of frozen tests/r8/judge-r8.py
SHA256 f021048da3c1b729c6f9bf560eba52609b2f980336dd4fab77ad1700438c0e31.
Do not overwrite the frozen file; historical cells judged by it stay as-is.

Collector jsonl is the sole semantic observation stream. raw/logcat/ring are
provenance only and are never concatenated into counting rows.
...
def completeness(rows: list[dict], role: str) -> None:
    ...
    if len(begin) != 1:
        raise Verdict(INVALID, f"MISSING_BEGIN_{role}" if not begin else f"MULTI_BEGIN_{role}")
    if len(end) != 1:
        raise Verdict(INVALID, f"MISSING_END_{role}" if not end else f"MULTI_END_{role}")
...
def judge(ev: Path, cell: str, manifest: dict | None) -> tuple[int, str]:
    try:
        ...
        ok, reason = collector_provenance(text, xobs + robs)
        if not ok:
