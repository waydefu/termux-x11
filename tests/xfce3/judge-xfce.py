#!/usr/bin/env python3
"""XFCE-FREEZE-V3 judge: xfce-run.json -> verdict. Implements freeze sections
...
def judge(run: dict, freeze: dict) -> dict:
    ...
    # V3: the product topology - X not ptrace-traced (RCA-XFCE-2) - and a screen that
    # stayed on (probe-03 ran dark: surface 1200x2464, 0 frames, composites never reached Gate A)
    validity.append(chk("x3_untraced", run.get("x3_tracer_pid") == 0, run.get("x3_tracer_pid")))
    scr = [run.get("screen_pre") or {}, run.get("screen_post") or {}]
    validity.append(chk("screen_awake_throughout",
                        all(s.get("awake") is True and s.get("keyguard") is False for s in scr), scr))
    # V2: the product, not the diagnostic build (PGA-GAP-1): zero p2a2 stamps from X
    validity.append(chk("diag_stamps_absent", run.get("diag_stamp_lines") == 0,
                        run.get("diag_stamp_lines")))
    ...
    # V2 split: "never exited" and "exited non-zero" are different facts. xfce-c1-03 on
    # dc94485 was the first kind (rc None: X starved, the pty was never drained) and V1
    # could only call it terminals_exit_0.
    closes = [s for s in steps if s.get("op") == "close"]
    term_rc = [s.get("rc") for s in closes]
    crash.append(chk("terminals_exited", bool(closes) and all(rc is not None for rc in term_rc),
                     term_rc))
    crash.append(chk("terminals_exit_code_0",
                     (all(rc == 0 for rc in term_rc if rc is not None)) if closes else False,
                     term_rc))
