#!/usr/bin/env python3
"""Static gate for XFCE-FREEZE-V3. Host only. Run before any XFCE run is granted.
...
def freeze_checks(f: dict) -> list[tuple[str, bool, object]]:
    ...
    need("status", f.get("status") == "XFCE_DESIGN_FROZEN_V3", f.get("status"))
    ...
    b = f["binding"]
    need("binding_filled", all(isinstance(v, (str, int)) and "__" not in str(v) for v in b.values()), b)
    need("binding_apk_sha_shape", re.fullmatch(r"[0-9a-f]{64}", str(b.get("apk_sha256"))) is not None)
    return out
...
def product_checks(f: dict) -> list[tuple[str, bool, object]]:
    ...
    dx = (LORIE.parent / "patches" / "dix-config.h.in").read_text(errors="replace")
    me = re.search(r"static void p2a2_emit\(const char \*msg\)\n\{.*?\n\}\n", dx, re.S)
    need("pga_gap1_gate_in_p2a2_emit", me is not None and "!p2a2_diag_enabled()" in me.group(0))
    need("pga_gap1_flag_exact_1", 'getenv("TERMUX_X11_P2A_DIAG")' in dx
         and "e[0] == '1' && e[1] == '\\0'" in dx)
...
def selftest(f: dict) -> int:
    ...
    m("binding placeholder", lambda g: g["binding"].__setitem__("apk_sha256", "__APK_SHA256__"),
      "binding_filled")
    bad = 0
