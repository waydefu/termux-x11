#!/usr/bin/env python3
"""XFCE-FREEZE-V3 collector: one run directory -> xfce-run.json. Derives, never judges.
...
# p2a2_emit stamp prefixes (PGA-GAP-1). With TERMUX_X11_P2A_DIAG unset the product
# prints none of them; one of them in a run means the diagnostic build was measured.
STAMP_PREFIXES = ("Probe ENTER", "Probe RETURN", "InstallProbe", "Gcomp ", "Sprep", "Sfb ",
                  "E0 ", "E0u ", "E1 ", "D0 ", "D1 ", "D2 ", "D3 ", "R3 S", "Sbt ")
...
def read_kv_colon(p: Path) -> dict:
    """/proc/<pid>/status style 'Key:\tvalue' lines."""
    out = {}
    try:
        for line in p.read_text().splitlines():
            if ":" in line:
...
def collect(ev: Path, freeze: dict) -> dict:
    ...
    for name in ("run-binding.json", "session-ready.json", "logout.json", "close.json",
                 ...
                 "capture-end.json", "screen-pre.json", "screen-post.json"):
    ...
    tr = read_kv_colon(ev / "x3-tracer.txt")
    out["x3_tracer_pid"] = _int(tr.get("TracerPid"))
