#!/usr/bin/env python3
"""Offline tests for the XFCE-FREEZE-V1 collector + judge (and the choreo driver's
...
class Run:
    ...
    def write_logcat(self):
        ...
        for i in range(12):
            L.append(lc(T0_EPOCH + 20 + i, X, "LorieNative", f"Probe ENTER depth=1 max=1 enter={i} return={i}"))
            L.append(lc(T0_EPOCH + 20 + i, STABLE, "LorieNative", f"Probe ENTER depth=1 max=1 enter={i} return={i}"))
        L.append(lc(T0_EPOCH + 30, X, "LorieNative", "Gcomp FDCLONE id=9 px0=00000000"))
...
class Clean(Base):
    ...
    def test_x_pid_filter_excludes_stable(self):
        ...
        self.assertEqual(run["composite_total"], 12)
